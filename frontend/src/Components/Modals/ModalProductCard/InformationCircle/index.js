...
const InformationCircle = props => {
  ...
  useEffect(() => {
    if (props.comboProducts)
      setProducts(
        Object.keys(props.comboProducts).reduce(
          (products, id) =>
            (!props.comboProducts[id].category ||
              props.comboProducts[id].category === 'snacks') &&
            !Object.values(products).find(
              item =>
                item &&
                item._id === props.comboProducts[id]._id &&
                item.thickness === props.thicknessObj[id]
            )
              ? {
                  ...products,
                  [id]: {
                    ...props.comboProducts[id],
                    thickness: props.thicknessObj[id]
                  }
                }
              : products,
          {}
