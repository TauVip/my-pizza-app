...
const InformationCircle = props => {
  ...
  useEffect(() => {
    props.comboProducts &&
      setProducts(
        props.comboProducts.reduce(
          (products, product, i) =>
            (!product.category || product.category === 'snacks') &&
            !products.find(
              item =>
                item &&
                item._id === product._id &&
                item.thickness === props.thicknessObj[i]
            )
              ? [...products, { ...product, thickness: props.thicknessObj[i] }]
              : [...products, null],
          []
        )
      )
              ...
              products.map(
                (product, i) =>
                  ...
                  (!product.category ? (
                    <InfoCirclePopup
                      pizza={product}
                      thickness={props.thicknessObj[i]}
                    ...
                    <InfoCirclePopup product={product} combo={true} key={i} />
                  ))
              )
            ) : (
