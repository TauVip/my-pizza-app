...
const ComboProductSection = props => {
  ...
  const pizza = pizzas?.find(findItem)
  const product = products[props.item.category]?.find(findItem)
  ...
  return (
    <div
      ...
      <img
        src={pizza ? pizza.images.traditional[props.item.size] : product.image}
        alt='Product'
      ...
      <div style={{ marginLeft: 76 }}>
        <div className='combo-product__title'>{(pizza || product).name}</div>
